...
export async function GET() {
  try {
    ...
    const existingTasksRes = await supabaseAdmin.from('tasks').select('*');
    ...
    if (tasks.length > 0) {
      await supabaseAdmin.from('tasks').upsert(tasks);
    }

    return NextResponse.json({
      success: true,
      message: `Synced ${memories.length} memories, ${docs.length} documents, ${tasks.length} tasks`,
      time: now,
      tasks,
    });
  } catch (error) {
